...
from utils.eval_utils import linear_model_eval_wrapper, plot_clusters, translate_to_new_domain, append_tensors_to_lists, \
    concatenate_lists
...
def evalulate_models(data_loader, model, config, plot_suffix="_Test", mode='train'):
    ...
    for i, (ds1_dict, ds2_dict, ds3_dict) in train_tqdm:
        ...
        Xtran_to_ds1_l, ztran_to_ds1_l = translate_to_new_domain(autoencoder, config, Xdata, Xtran_to_ds1_l,
                                                                 ztran_to_ds1_l, to_domain=0)
        # Translate all input data to Domain-2
        Xtran_to_ds2_l, ztran_to_ds2_l = translate_to_new_domain(autoencoder, config, Xdata, Xtran_to_ds2_l,
                                                                 ztran_to_ds2_l, to_domain=1)
        # Translate all input data to Domain-3
        Xtran_to_ds3_l, ztran_to_ds3_l = translate_to_new_domain(autoencoder, config, Xdata, Xtran_to_ds3_l,
                                                                 ztran_to_ds3_l, to_domain=2)
    ...
    # Visualise clusters
    plot_clusters(config, z, ztran_to_ds3, clabels, dlabels, plot_suffix="_inLatentSpace_" + plot_suffix)

    # The Linear model evaluation
    linear_model_eval_wrapper(config, Xdata, Xrecon, Xtran_to_ds1, Xtran_to_ds2, Xtran_to_ds3, clabels, dlabels,
                              mode=mode, save_files=config["save_files"])
