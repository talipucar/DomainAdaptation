...
def linear_model_eval_wrapper(Xdata, Xrecon, Xtran_to_ds1, Xtran_to_ds2, Xtran_to_ds3, clabels, dlabels_raw, save_files=False):
    ...
    if save_files:
        ...
        save_np2csv([Xds1_to_ds2, clabels_ds1], save_as="./results/evaluation/clusters/ds1_to_ds2.csv")
        save_np2csv([Xds2_to_ds1, clabels_ds2], save_as="./results/evaluation/clusters/ds2_to_ds1.csv")
...
def linear_model_eval(X_train, y_train, X_test, y_test, use_scaler=False, description="Baseline: PCA + Logistic Reg."):
    ...
    clf = LogisticRegression(random_state=0, max_iter=1200, solver='lbfgs', C=0.1)
...
def plot_clusters(z, ztran_to_ds3, clabels, dlabels_raw, plot_suffix="_inLatentSpace"):
    ...
    # Show domains only
    visualise_clusters(z, dlabels_raw, plt_name="domains" + plot_suffix, legend_title="Domains", legend_labels=dlegends)
    # Show cohorts only
    visualise_clusters(z, clabels, plt_name="cohorts" + plot_suffix, legend_title="Cohorts", legend_labels=clegends)
    # Show both cohorts and domains
    visualise_clusters(z, new_labels, plt_name="domain_cohorts" + plot_suffix, alpha=1.0, legend_title="Cohorts/Domains", ncol=4)

    # Plot translations from all domains to a particular domain (e.g. ds3)
    # Show both cohorts and domains
    visualise_clusters(ztran_to_ds3, new_labels, plt_name="domains_cohorts_translations" + plot_suffix, alpha=1.0, legend_title="Cohorts/Domains", ncol=4)
    # Show domains only
    visualise_clusters(ztran_to_ds3, dlabels_raw, plt_name="domains_translations" + plot_suffix, legend_title="Domains", legend_labels=dlegends)
    

def visualise_clusters(embeddings, labels, plt_name="test", alpha=0.2, legend_title=None, legend_labels=None, ncol=1):
    """
    ...
    color_list = ['#66BAFF', '#FFB56B', '#8BDD89', '#faa5f3',
                  '#008cff', '#ff8000', '#04b000', '#de4bd2',
                  '#004c8b', "#964b00", "#026b00", "#ad17a1",
                  "#00325c", "#e41a1c", "#008DF9", "#570950", "#00FB1D"]
    
    color_list2 = ['#66BAFF', '#008cff', '#004c8b', '#00325c',
                  '#FFB56B', '#ff8000', '#964b00', '#e41a1c',
                  '#8BDD89', "#04b000", "#026b00", "#008DF9",
                  "#faa5f3", "#de4bd2", "#ad17a1", "#570950", "#00FB1D"]
    
    # If there are more than 3 types of labels, we want to plot both cohort, and domains, so change color scheme.
    color_list = color_list2 if len(list(set(labels)))>4 else color_list
    ...
    sns_plt = sns.scatterplot(x=embeddings_pca[:, 0], y=embeddings_pca[:, 1], ax=axs[0], palette=palette, hue=y, s=20, alpha=alpha)
    ...
    # Plot samples, using each class label to define the color of the class.
    sns_plt = sns.scatterplot(x=embeddings_tsne[:, 0], y=embeddings_tsne[:, 1], ax=axs[1], palette=palette, hue=y, s=20, alpha=alpha)
    # Overwrite legend labels    
    overwrite_legends(sns_plt, c2l, fig, ncol=ncol, title=legend_title, labels=legend_labels) 
